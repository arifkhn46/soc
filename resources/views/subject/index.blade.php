                                  ...
                                  <!-- <td>
                                      <ul class="menu-list">
                                        @if(!$subject->deleted_at)  
                                            <li>
                                                <a class="button" href="{{ route('courses.edit', $subject->id) }}">Edit</a>
                                            </li>
                                            <li>
                                                <a class="button" href="#" 
                                                onclick="event.preventDefault();
                                                        document.getElementById('course-unpublish-form-{{ $subject->id }}').submit();">Unpublish</a>
                                                <form id="course-unpublish-form-{{ $subject->id }}" action="{{ route('courses.delete', $subject->id) }}" method="POST" style="display: none;">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}
                                                </form>
                                            </li>
